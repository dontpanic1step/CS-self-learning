...
class Ant(Insect):
    ...
    def remove_from(self, place):
        if place.ant is self and place.ant.name == 'Queen' and place.ant.id == 1:
            return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p = self.place
        step = 0
        while not p.is_hive() and step <= self.max_range:
            if len(p.bees) > 0 and self.min_range <= step:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if not self.chewing and len(self.place.bees) > 0:
...
class TankAnt(ContainerAnt):
    """TankAnt protects the ant it contains and attacks all the bees in its place."""
    name = 'Tank'
    food_cost = 6
    damage = 1
    implemented = True
