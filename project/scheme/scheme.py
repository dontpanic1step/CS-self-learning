...
def do_if_form(expressions, env):
    ...
    3
    My note: The second and the third argument in if expression is in tail context.
    """
...
def do_and_form(expressions, env):
    ...
    4
    False
    Mynote: The last sub expression in and expression is in tail context.
    """
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    if expressions is nil:
        return True
    while expressions is not nil:
        ans = scheme_eval(expressions.first,env,True if expressions.rest is nil else False)
...
def do_or_form(expressions, env):
    ...
    2
    6
    Mynote: The last sub expression in or expression is in tail context.
    """
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    if expressions is nil:
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    "*** YOUR CODE HERE ***"
    formals = expressions.first.first
    body = expressions.rest
    env.define(formals,MacroProcedure(formals,body,env))
    return formals
    # END Problem 20
...
def complete_apply(procedure, args, env):
    ...
    if isinstance(val, Thunk):
        # My note: If it's a Thunk, use scheme_eval to evaluate it.
        # Ensure the answer is evaluated completely.
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            # My note:
            # This is in tail context. Don't evaluate it now, leave it in Thunk,
            # which will be evaluated in env later.
        ...
        "*** YOUR CODE HERE ***"
        # My note:化递归为迭代
        # Thunk代表延迟求值，所以反复调用original_scheme_eval，直到tail=True，允许result求值
        # 这么做的目的是为了减少递归时frame占用，最后溢出
