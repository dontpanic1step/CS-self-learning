...
def accuracy(typed, reference):
    ...
    if n == 0:
        ...
        correct_num = 0
        if n <= len(reference_words):
            n2 = 0
            for w1 in typed_words:
                if w1 == reference_words[n2]:
                    correct_num += 1
                n2 += 1
        else:
            n1 = 0
            for w2 in reference_words:
                if w2 == typed_words[n1]:
                    correct_num += 1
                n1 += 1
        return correct_num / n * 100
    # END PROBLEM 3
...
def report_progress(typed, prompt, user_id, send):
    ...
    for word in typed:
        if word == prompt[n2]:
            ...
            break
        n2 += 1
    correct_prob =  correct_count/len(prompt)
    dic = {"id":user_id,"progress":correct_prob}
    send(dic)
    return correct_prob
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(word_num):
           newtime.append(player[i+1]-player[i])
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(game,j,i) < time(game,min_time_player,i):
                min_time_player = j
        records[min_time_player].append(word_at(game,i))
    return records
    # END PROBLEM 10
