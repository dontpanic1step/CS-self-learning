...
def next_perfect_square(score):
    assert perfect_square(score),"Not a perfect square"
    for i in range(score+1):
        if i*i == score:
            return (i+1)*(i+1)
# END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    maxscore = 0
    for i in range(1,11):
        newscore =  make_averaged(roll_dice,total_samples)(i,dice)
        if newscore > maxscore:
            ans,maxscore = i,newscore
    return ans
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    nextscore = square_update(0,score,opponent_score)
    if nextscore - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    use tail strategy if the increase is greater than 10.
    use square strategy if the increase is greater than 10.
    otherwise, return 6.
    """
    # BEGIN PROBLEM 12
    if tail_points(opponent_score) >= 10 or square_update(0,score,opponent_score)-score >= 10:
        return 0
    else:
        return 6
    # END PROBLEM 12
